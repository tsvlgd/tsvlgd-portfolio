...
(function oneko() {
  ...
  let dragOffsetX = 0;
  let dragOffsetY = 0;
  // Track dragging to prevent accidental click immediately after
  ...
  function init() {
    ...
    document.addEventListener("mouseup", () => {
      if (isDragging) {
        isDragging = false;
        nekoEl.style.cursor = "pointer";
        isStopped = true; // Drop and stay stopped
      }
    });

    nekoEl.addEventListener("click", (e) => {
      e.stopPropagation();
      // Only toggle state if we didn't just finish dragging
      if (!hasDragged) {
        isStopped = !isStopped;
      }
      hasDragged = false;
